    ...
    if (self._is_server_ready) {
      ...
      socket.on ('set_event', function (data) {
        self._set_data (route, data, function (value) {
          socket.emit ('value_event', value);
          socket.broadcast.to ('VAL_EVT').emit ('value_event', value);
        });
      });
      socket.on ('push_event', function (data) {
        self._push_data (route, data, function (child, value) {
          socket.emit ('child_added_event', child);
          socket.broadcast.to ('CHLD_ADD_EVT').emit ('child_added_event', child);
          socket.broadcast.to ('VAL_EVT').emit ('value_event', value);
        });
      });
    } else {
