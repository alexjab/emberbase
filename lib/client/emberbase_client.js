var Reference = function (socket, route, path) {
  this._socket = socket;
  this._route = route;
  this.path = path;
...
Reference.prototype.set = function (data) {
  this._socket.emit ('set_event', {path: this.path, data: data, _route: this._route});
  return this;
};

Reference.prototype.push = function (data) {
  this._socket.emit ('push_event', {path: this.path, data: data, _route: this._route});
  return this;
};
...
Reference.prototype.on = function (event, callback) {
  if (event === 'value') {
    ...
    this._socket.emit ('CLIENT_VALUE', {_route: this._route});
    ...
    this._socket.emit ('CLIENT_CHILD_ADDED', {_route: this._route});
...
Reference.prototype.child = function (path) {
  return new Reference (this._socket, this._route, path);
};
...
var Emberbase = function (url, username, password) {
  ...
  this._socket.on ('reconnect', function () {
    ...
    this._socket.on ('JOIN_ROUTE_ACK', function () {
      if (this._client_value)
        this._socket.emit ('CLIENT_VALUE', {_route: route});
      if (this._client_child_added)
        this._socket.emit ('CLIENT_CHILD_ADDED', {_route: route});
    }.bind (this));
  }.bind (this));
  ...
  this._socket.on ('UNKNOWN_ROUTE_ERR', function (data) {
    throw new Error ('The route '+data.route+' does not exist on the server');
  });
  ...
  var mainRef = new Reference (this._socket, route, '');
