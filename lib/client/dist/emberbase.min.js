...
var Emberbase=function(e){var t=e.split("/").slice(0,3).join("/"),s=e.split("/").slice(3,4).join("/");this.socket=io.connect(t),this.socket.emit("JOIN_ROUTE_MSG",{route:s}),this.serverReady=!1,this.socketReady=!1;var o=this;return this.socket.on("SRV_NOT_RDY_ERR",function(){throw new Error("The server is not ready to accept data")}),this.socket.on("SRV_RDY_MSG",function(){o.serverReady=!0}),this.socket.on("WRNG_NS_ERR",function(){throw new Error("The route "+s+" does not exist on the server")}),this.socket.on("JOIN_ROUTE_ACK",function(){o.socketReady=!0}),this};Emberbase.prototype.set=function(e){if(!this.serverReady)throw new Error("The server is not ready to accept data");if(!this.socketReady)throw new Error("The route you specified does not exist on the server");return this.socket.emit("set_event",e),this},Emberbase.prototype.push=function(e){if(!this.serverReady)throw new Error("The server is not ready to accept data");if(!this.socketReady)throw new Error("The route you specified does not exist on the server");return this.socket.emit("push_event",e),this},Emberbase.prototype.on=function(e,t){switch(e){case"value":this.socket.emit("CLT_VAL_EVT_MSG");break;case"child_added":this.socket.emit("CLT_CHLD_ADD_EVT_MSG")}return this.socket.on(e+"_event",t),this};