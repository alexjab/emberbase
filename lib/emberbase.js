...
Emberbase.prototype._onJoinRoute = function (socket, data) {
  if (this.routes.indexOf (data.route) === -1) {
    socket.emit ('UNKNOWN_ROUTE_ERR', {route: data.route});
  } else {
    socket.join (data.route);
    socket.emit ('JOIN_ROUTE_ACK');
    if (data.auth) {
...
Emberbase.prototype._onClientValue = function (socket, data) {
  var route = data._route;
  if (this.routes.indexOf (route) === -1) {
    socket.emit ('UNKNOWN_ROUTE_ERR', {route: route});
  } else {
...
Emberbase.prototype._onClientChildAdded = function (socket, data) {
  var route = data._route;
  if (this.routes.indexOf (route) === -1) {
    socket.emit ('UNKNOWN_ROUTE_ERR', {route: route});
  } else {
  ...
  if (this.io) {
    this.io.on ('connection', function (socket) {
      ...
      if (this.serverStatus === 'ready') {
        ...
        socket.on ('CLIENT_VALUE', function (data) {
          this._onClientValue.call (this, socket, data);
        }.bind (this));
        socket.on ('CLIENT_CHILD_ADDED', function (data) {
          this._onClientChildAdded.call (this, socket, data);
        }.bind (this));
...
Emberbase.prototype._pushData = function (route, commit, callback, context) {
  ...
  var fullPath = (path?(path+'/'):'')+id;


  utils.setReference (this._data, route, fullPath, data, function (put, del) {
    if (put) {
      utils.flattenObject (route, fullPath, put, function (op) {
        if (this._useDB) {
    ...
    if (del) {
      utils.flattenObject (route, fullPath, del, function (op) {
        if (this._useDB) {
  ...
  callback.call (context, {key: id, val: data, path: path}, {path: path, val: this._data[route]});
...
Emberbase.prototype._setData = function (route, commit, callback, context) {
  ...
  callback.call (context, {path: path, val: this._data[route]});
  ...
  if (this._data[route]) {
    if (typeof this._data[route] === 'object') {
      Object.keys (this._data[route]).forEach (function (key) {
        callback.call (context, {key: key, val: this._data[route][key], path: ''});
      }, this);
    }
  }
  return this;
...
Emberbase.prototype._getValue = function (route, callback) {
  callback ({val: this._data[route], path: ''});
  return this;
};
